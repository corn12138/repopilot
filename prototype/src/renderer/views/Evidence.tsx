 ...
 * 那需要 SPK-010 的 sealed A/B —— 页面上把这句话原样说给用户听。
 */
...
function DistTable({ dist }: { dist: Readonly<Record<string, number>> }) {
...
export function EvidenceView({ onError }: { onError: (message: string, detail: string | null) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Banner tone="info">
        本页全部数字来自平台观察到的事实（账本 / append-only 事件 / 封存记录），
        <b>不从模型或审核方的自报计算</b>。它是观察性分布，
        <b>不构成 ASM-019（异构审核是否降低缺陷）的证据</b> —— 那需要 SPK-010 的 sealed A/B 对照。
      </Banner>
      ...
      <Card title="北极星与漏斗" hint="两者必须同屏发布 —— 只看接受率会隐藏前置阶段的失败（PRD §11.1）">
        <dl className="kv">
          <CountRow label="Accepted Verified Patch Rate" value={pct(ns.rate)} />
          <CountRow label="分子：接受且验证通过（SUCCEEDED）" value={ns.acceptedVerified} />
          <CountRow label="接受但未验证（ACCEPTED_UNVERIFIED）" value={ns.acceptedUnverified} />
          <CountRow label="分母：进入 EXECUTING 的 Attempt" value={ns.executingAttempts} />
        </dl>
      ...
      <Card title="终态与失败分类">
        <dl className="kv">
          <dt>终态分布</dt>
          <dd>
            <DistTable dist={summary.outcomes.byStatus} />
          </dd>
          <dt>失败分类</dt>
          <dd>
            <DistTable dist={summary.outcomes.byFailureClass} />
          </dd>
        </dl>
      </Card>
          ...
          summary.crossReview.groups.map((g) => (
            <div key={`${g.reviewerKey}|${g.parity}`} style={{ marginTop: 10 }}>
              ...
              <dl className="kv">
                ...
                <CountRow
                  label="结论分布"
                  value={`PASS ${g.verdicts.PASS} · 要求修改 ${g.verdicts.CHANGES_REQUESTED} · 未给出结论 ${g.verdicts.INCONCLUSIVE}（后者不是通过）`}
                />
