...
- **诚实边界**：案例集目前只有 **2 个 seed**（钉 harness 正确性用），离 20 个的样本
  门槛还远；正式实验（真实模型 + 人工盲评）按协议需单独确认，**还没有跑**。
  harness 建成 ≠ 实验做完，「一写一审值不值」仍是 Open。
...
- **编辑器写路径**：编辑器目前是查看器；用户手改如何走权威（receipt / CAS / 审计）
  还没设计。
- **SPK-010 正式实验**：案例集 2/20，真实模型 pilot 与人工盲评未跑。
