...
**9/22 复审后的继续路线**：先收紧审核请求契约和补丁完整性，再修审核计数、未结算用量恢复、会话快照竞态与启动中断，随后贯通任务工作台。状态与恢复问题、任务旅程和真实双模型验收仍未完成；现有正式测试通过不代表这些已知缺口关闭。首批实现见 [开发记录](docs/devlog/2026-09-22-01-审核契约和补丁交付必须可验证.md)。
...
- 修复后必须是真的 `EXIT_ZERO`
- 锁文件、导入时已有的生成源码及显式 mutation 均保留交付；只排除其余约定输出目录文件，并披露新增、修改与删除
- 审核实际请求声明四个只读工具及 `submit_review`；两条 wire 的截断、未知结束原因均不能提交结论
- 补丁 diff 里不含宿主绝对路径
...
观察页还把机器字段显示本轮结束的会话集中到“本轮结束可查看”队列。队列每 3 秒刷新，
...
**双 Agent 工作台已经接入产品种子。** Claude/Codex 双栏各自使用独立的 typed IPC、
隔离进程和流事件；能力实测为 `SUPPORTED` 时可以创建平台管理的新会话、发送输入、
请求取消并查看能力证据，未验证能力保持禁用。句柄按项目归属，回放与活事件按单调序号合并；
已实现项目归属检查与切换清理；同项目迟到快照覆盖活状态的竞态仍待修复。
工作台按 herdr 的思路把 **agent 注意力状态**（工作中 / 等你批准 / 等你输入 / 本轮完成 / 空闲 / 断开 / 出错）
做成一等公民：它独立于连接态，从每个会话汇到项目级的「需要你处理」计数，跨项目只汇总数字、不串内容；
同一 vendor 可并存多个会话（不再是一栏一个）。状态只来自结构化引擎事件，不读屏、不 GUI 自动化。
**blocked 只呈现「谁在等你答话」、不代答**（应答发生在会话自己的工具里）；Codex 受管会话固定
`approvalPolicy=never` + 只读，故不产出审批 blocked（负向断言钉住隔离没被放宽），Claude conversation-only
下权限映射已接线、真实产出待引擎模式验收。
双 Agent 任务把计划方、实施方、审核方分别冻结，并支持人工交接与有界自动两种模式。
人工模式在验证失败→自修复、实施→首审、发现→整改、整改→复审四个边界停靠，只有用户确认带 digest 的
交接工件后才调用下一角色。工作台直接展示 Core 冻结角色、当前循环 2/1、Task 累计、自修复、未知用量，
并在任务详情抽屉复用审批、交接、发现、验证和补丁决定入口。当前机器证据覆盖协议、状态机和零模型探针；
真实凭据回复、已有 Desktop 活跃会话接管仍未验证；Electron 已完成 1440×1024 与 760×900 实际布局检查。详见
[接入验证记录](docs/devlog/2026-09-15-02-双工位先验证会话协议.md)、
[协作流程规划记录](docs/devlog/2026-09-15-03-交接点由Core决定.md)与
[实现记录](docs/devlog/2026-09-15-04-工作位能启动不等于模型已对话.md)、
[状态感知记录](docs/devlog/2026-09-18-01-把谁在等你答话做成一等状态.md)。

## 一写一审：交叉审核与外部 CLI

补丁封存后，可以让**第二个模型**只读审一遍，产出结构化发现（severity /
confidence / file / range / evidence / blocking）。审核方由平台强制只读，
拿不到写工具。外部 CLI 路径会拒绝可证明的同厂商组合；现有模型 API 路径仍允许同厂商
披露后审核，无法判断厂商时显示 `UNVERIFIABLE`。双 Agent 协作模式统一要求实施方与
审核方可证明异构，无法证明时在任务创建前拒绝。
...
[`docs/devlog/`](docs/devlog/) 按天记录设计取舍和踩过的坑，目前 **87 篇**，
