...
`openai-compatible.ts` 的 `toWireMessages` 是按**块类型**拆的，不是按消息拆的：
...
所以我那两条连续 user 消息里，第一条是纯 tool_result → 变成 `role:'tool'`，
第二条是纯 text → 变成 `role:'user'`。OpenAI 侧看到的是
`assistant(tool_calls)` → `tool` → `user`，**完全合法**。
...
`agent.ts` 里 7 处 user push 全部改用它（另外还剩两处裸的 `role: 'user'`，
那是 `runAgent` 与 `runReviewPass` 各自 conversation 数组的**初始化** ——
第一条消息不存在"合并进上一条"的问题，本来就不该走 pushUser）。
包括几处**目前不会出问题**的：
...
在一处 `??` 上接一下**，所有接了总入口的调用点自动生效。昨天那版是一个具体检查
散布在 7 个文件里，补的时候得挨个改。

**注意"所有调用点"这个说法当时不成立。** 写这篇时我漏掉了交叉审核套件里的两处守卫
（`agent.crossreview.test.ts` 的审核方替身和内联 gateway stub），它们还停在
`findOrphanToolUse`，所以审核通路上的替身拿不到 role 交替这条检查。
总入口只对**已经接进来的**调用点生效 —— 这正好又印证了下面那条"防线是照着上一次
出的事建的"。审核开发记录时才发现，现在 6 处守卫都改过来了。
...
上一篇：[两个 wire 的翻译层，以及它可以怎么骗你](2026-08-09-06-两个wire的翻译层.md)
回到：[开发记录目录](README.md)
